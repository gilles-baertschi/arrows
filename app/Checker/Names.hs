...
checkNameSafety = void $ runStateT f (Names [] [] [])
  where
    f = do
      ...
      mapM_ checkCycles programAliases
...
checkCycles :: TypeAlias -> ParserWithDoubleState Names Program ()
checkCycles (TypeAlias _ _ Nothing) = return ()
checkCycles (TypeAlias name _ (Just referentialType)) = setOffset (nameOffset name) >> check [(name, referentialType)] referentialType
  where
    check :: [(Name, ReferentialType)] -> ReferentialType -> ParserWithDoubleState Names Program ()
    check _ (ReferentialType (Product _ _) _) = return ()
    check _ (ReferentialType (Sum _ _) _) = return ()
    check seen (ReferentialType (AliasReference n arguments) oldReferences) = do
      (TypeAlias _ _ maybeType) <- lift $ getAlias n
      case maybeType of
        Nothing -> return ()
        (Just (ReferentialType unincreasedMainType otherTypesWithPlaceholders)) -> do
          let otherTypesWithoutPlaceholders = drop (length arguments) otherTypesWithPlaceholders
          let amount = length oldReferences
          ...
          let newReferentialType = ReferentialType increasedMainType (oldReferences ++ arguments ++ newReferences)
          when ((n, newReferentialType) `elem` seen) $ fail $ "cycle detected\n" ++ unlines (displayReferentialType . snd <$> reverse ((n, newReferentialType) : seen)) ++ "..."
          check ((n, newReferentialType) : seen) newReferentialType
    check _ (ReferentialType (ForAllInstances _ _) _) = return ()
    check _ (ReferentialType (AnyType _ _) _) = return ()
    check _ (ReferentialType (AliasExtention _ _) _) = return ()
    check seen (ReferentialType (TypeReference index) references) = check seen (ReferentialType (references !! index) references)
    check _ (ReferentialType ThisClass _) = return ()
