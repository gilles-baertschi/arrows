    ...
    case eitherReferentialTypes of
        ...
        (Right fromInstances) -> do
            ...
            possibleIndecies <-
                concat
                    <$> mapM
                        ( \(index, fromInstance) -> observingFail
                             ( do
                                 put backup
...
observingFail p = do
    r <- observing p
    return $ case r of
        (Left (FancyError _ xs)) -> map (\x -> case x of
...
typeGreaterThan inferior@(AnyType inferiorIndex existingClasses) superior@(AnyType superiorIndex additionalClasses) = do
